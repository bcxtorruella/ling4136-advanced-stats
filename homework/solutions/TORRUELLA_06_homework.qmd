...
ggplot(data = sim) +
  geom_density(aes(x = Distance, fill = Sem, alpha = 0.5))
```
...
# normal is (mean, sd)?
# class of SemSimilar is "b" What if there are several b's?
my_priors <- c(
  prior(normal(107,30), class = "Intercept"),
  prior(normal(0, 30), class = "b")
)

# ...should sigma be a prior as well?
```
...
# The data and posterior samples do broadly align. The peaks are in the same place on the x-axis and their widths are very similar. There does appear to be some bimodality (trimodality?) in the data that the model doesn't reflect, which would explain why the posterior samples tend to be a little lower on the y-axis and to the right of the data.
...
# try log transform, why not
sim_log <- sim %>% mutate(log_Distance = log(Distance))
ggplot(data = sim_log) +
  geom_density(aes(x = log_Distance, fill = Sem, alpha = 0.5))

#redo model
log_mdl_formula = bf(Distance ~ Sem)
log_my_priors <- c(
  prior(lognormal(4.4, 1), class = "Intercept"), # change this to log as well? Just try a value that looks like the center...I'm not 100% sure how to control this distribution. 
  prior(normal(0, 30), class = "b")
)
log_xmdl <- brm(log_Distance ~ Sem,
            family = gaussian(),
            prior = log_my_priors,
            data = sim_log,
            seed = 42)
...
# redo the check
pp_check(log_xmdl, ndraws = 100)

# Not really any better. But the log transform didn't fix all the bimodality, just made it switch which side of the peak it appeared on. Interesting.
# I don't know what other "transformation" might fix this. Centering? Standardization? It looks like it's just part of the data.
```
...
# RESULTS
# with absurdly tiny SDs for both, no effect is found
# with a tiny intercept and huge effect mean, the opposite effect is found (different is closer, +10 on the difference graph)
# with a huge intercept and huge effect mean, same--opposite effect
# with a tiny intercept and tiny effect mean, you get a correctly directed effect, but different estimates
# with a huge intercept and tiny effect mean, same--different estimates, same direction
# SOOO it seems like the magnitude of the effect mean (rather than the intercept) can dictate some directionality to the effect--maybe it matters whether it is very much higher or very much lower that reasonable? Furthermore, SD is what clearly answers this exercise and removes a predicted difference.

evil_priors <- c(
  prior(normal(107,0.1), class = "Intercept"),
  prior(normal(0, 0.1), class = "b")
  #prior(normal(999,30), class = "Intercept"),
  #prior(normal(0.1, 30), class = "b")
)

evil_xmdl <- brm(Distance ~ Sem,
            family = gaussian(),
            prior = evil_priors,
            data = sim,
            seed = 42)
...
# check the difference between conditions. Should be 0 now.
evil_xmdl |> 
  # first extract the samples for the differences only
  spread_draws(b_SemSimilar) |> 
  # plot
